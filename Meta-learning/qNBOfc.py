...
def main():
    ...
    parser = argparse.ArgumentParser(description='qNBO with Partial Parameter Adaptation')
    parser.add_argument('--run_id', type=int, default=1, help='which run this is, e.g. 1,2,3,4,5')
    parser.add_argument('--seed', type=int, default=None, help='random seed; if not set, use run_id')
    parser.add_argument('--dataset', type=str, default='fc100', metavar='N', help='omniglot or miniimagenet or fc100')
    ...
    log_name = f'log_qNBO_{args.dataset}_run{run}.txt'
    ...
    if args.dataset == 'omniglot':
        ...
        meta_model = torch.nn.Sequential(l2l.vision.models.ConvBase(hidden=64, channels=3, max_pool=True),
    ...
    for k in range(start_iter, K):
        ...
        for t_idx in range(meta_bsz):
            ...
            final_params,et = bfgs(task, hparams, params, tol=1/(10*(t_idx+1)),step=20, maxiter_hg=3,stepsize1=0.03,stepsize2=0.01,m=30,params0=w0,h0=0.01,device1=device)#T=50
        ...
        if (k+1) % log_interval == 0 or k == 0 or k == K-1:
            string = '[Run {}] META k={}/{} Lr: {:.5f} mu: {:.3f}  ({:.3f}s F: {:.3f}s, B: {:.3f}s, HG: {:.3f}s) Train Loss: {:.2e}, Train Acc: {:.2f}.'.format(
    run, k+1, K, outer_lr, mu, step_time, forward_time, backward_time, th, val_loss, 100. * val_acc
)
            args.out_file.write(string + '\n')
            args.out_file.flush()
            print(string)
        ...
        if (k+1) % args.save_every == 0:
            ...
            filename = f'qNBO_{args.dataset}_run{run}.pt'
        ...
        if (k+1) == stop_k: # early stopping
            ...
            filename = 'FOAcon' + args.dataset+  '05114.pt'
            save_path = os.path.join(args.ckpt_dir, filename)
        ...
        if (k+1) % eval_interval == 0:
            ...
            string = "[Run {}] Val loss {:.2e} (+/- {:.2e}): Val acc: {:.2f} (+/- {:.2e}) [mean (+/- std) over {} tasks].".format(
    run, val_losses.mean(), val_losses.std(), 100. * val_accs.mean(), 100. * val_accs.std(), len(val_losses)
)
            args.out_file.write(string + '\n')
            args.out_file.flush()
            print(string)
            ...
            string = "[Run {}] Test loss {:.2e} (+/- {:.2e}): Test acc: {:.2f} (+/- {:.2e}) [mean (+/- std) over {} tasks].".format(
    run, test_losses.mean(), test_losses.std(), 100. * test_accs.mean(), 100. * test_accs.std(), len(test_losses)
)
            args.out_file.write(string + '\n')
            args.out_file.flush()
            print(string)

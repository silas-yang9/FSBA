...
def g_xx_hessian(x,w,dataset):
    ...
    for i in range(n):
        try:
            ...
            print(f"comupte gxx{i} row Hessian mistake: {e}")
...
def f_xx_hessian(x, w, dataset):
    ...
    for i in range(n):
        try:
            ...
            print(f"compute fxx{i} row Hessian misrake: {e}")
...
def jacobian_g_xw(xhat, w, trainset):
    ...
    for i in range(g_w_flat.numel()):
        ...
        try:
            ...
            print(f"compute {i} row jacobian mistake: {e}")
...
def RAHGD(args, x, w, trainset, valset, testset, clean_indices):
    """
    RAHGD: Regularized Accelerated Hypergradient Descent
    
    Key features:
    - Inner loop: AGD (Accelerated Gradient Descent) with momentum
    - Outer loop: AGD with momentum + CG for hypergradient
    - Restart mechanism: prevents oscillation when k*s > B²
    
    Each epoch contains args.iterations inner updates (improved version).
    """
    outer_loss = lambda x, w: f(x[0], w[0], valset)
    inner_loss = lambda x, w, d: g(x[0], w[0], d)
    ...
    # RAHGD hyperparameters
    tol = 1e-12
    inner_theta = 0.005  # Inner AGD momentum
    outer_theta = 0.005  # Outer AGD momentum
    B = 0.1              # Restart threshold
    ...
    w_x = w.clone().detach().requires_grad_(True)
    w_x0 = w_x.clone()
    x0 = x.clone()
    ...
    for epoch in trange(args.epochs):
        ...
        # ============================================
        # Inner optimization: AGD with multiple iterations
        # ============================================
        x_prev = x0.clone()
        
        for inner_iter in range(args.iterations):
            # AGD momentum step
            x_y = x + (1 - inner_theta) * (x - x_prev)
            
            # Compute gradient
            x_y.requires_grad_(True)
            inner_update = g_x(x_y, w_x, trainset)
            ...
            # Update
            x_prev = x.clone()
            x = x_y - args.x_lr * inner_update
            x = x.detach().requires_grad_(True)
        
        # ============================================
        # Outer optimization: AGD + CG
        # ============================================
        
        # AGD momentum step for w
        w_y = w_x + (1 - outer_theta) * (w_x - w_x0)
        w_y.requires_grad_(True)
        
        # Compute hypergradient using CG
        hg.CG([x], [w_y], args.hessian_q, inner_opt_cg, outer_loss, 
              stochastic=False, set_grad=True, tol=tol)
        
        # Update w
        w_x0 = w_x.clone()
        w_x = w_y - args.w_lr * w_y.grad
        w_x = w_x.detach().requires_grad_(True)
        
        # ============================================
        # Restart mechanism (prevent oscillation)
        # ============================================
        s += float(torch.norm(w_x - w_x0)) ** 2
        k += 1
        
        if k * s > B ** 2:
            w_x = w_x0.clone()
            k, s = 0, 0
            print(f"[RAHGD] Epoch {epoch}: Restart triggered")
        
        # Update main variables
        w.data = w_x.data.clone()
        w.data.clamp_(0.0, 1.0)
        x0 = x.clone()
...
def baselineRAHGD(args, x, w, trainset, valset, testset, clean_indices):
    """
    Baseline RAHGD: restarted accelerated hypergradient descent without perturbation.
    """
    outer_loss = lambda x, w: f(x[0], w[0], valset)
    inner_loss = lambda x, w, d: g(x[0], w[0], d)
    inner_opt_cg = hg.GradientDescent(inner_loss, 1., data_or_iter=trainset)
    ...
    w_x = w.clone().detach().requires_grad_(True)
    w_x0 = w_x.clone().detach().requires_grad_(True)
    ...
    for epoch in trange(args.epochs):
        ...
        for _ in range(args.iterations):
            ...
            if float(torch.norm(inner_update)) < tol:
                break
            x_prev = x
            x = (x_y - args.x_lr * inner_update).detach().requires_grad_(True)

        w_y = w_x + (1 - outer_theta) * (w_x - w_x0)
        hg.CG([x], [w_y], args.hessian_q, inner_opt_cg, outer_loss, stochastic=False, set_grad=True, tol=tol)
        if w_y.grad is None:
            raise RuntimeError("Failed to compute hypergradient in baselineRAHGD")

        w_prev = w_x
        w_x = (w_y - args.w_lr * w_y.grad).detach().requires_grad_(True)
        w_x0 = w_prev.detach().requires_grad_(True)

        s += float(torch.norm(w_x - w_prev)) ** 2
        k += 1
        if k * s > B ** 2:
            w_x = w_prev.detach().requires_grad_(True)
            w_x0 = w_prev.detach().requires_grad_(True)
            k, s = 0, 0

        w.data.copy_(w_x.data)
        w.data.clamp_(0.0, 1.0)
        x0 = x_prev.detach().requires_grad_(True)
